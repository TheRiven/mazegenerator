...
fn find_cell_neighbours<'a>(cell: &Cell, cells: &'a HashSet<Rc<Cell>>) -> Vec<&'a Rc<Cell>> {
    ...
    if let Some(north) = cells.get(&Cell { x: x, y: y - 1 }) {
        neighbour_list.push(north);
    }

    if let Some(east) = cells.get(&Cell { x: x + 1, y: y }) {
        neighbour_list.push(east);
    }

    if let Some(south) = cells.get(&Cell { x: x, y: y + 1 }) {
        neighbour_list.push(south);
    }

    if let Some(west) = cells.get(&Cell { x: x - 1, y: y }) {
